...
def build_app(orch: Orchestrator, settings: Settings | None = None) -> gr.Blocks:
    ...
    base_css = css_path.read_text(
        encoding="utf-8") if css_path.exists() else ""
    ...
    welcome_message = {"role": "assistant",
                       "content": "你好呀，我是中关三小校史讲解智能助手！有什么可以帮你的吗？"}
    ...
    with gr.Blocks(theme=gr.themes.Glass(), css=final_css, title="ZGC3 校园 AI 助手") as demo:
        ...
        with gr.Column(elem_id="main-container"):
            ...
            chatbot = gr.Chatbot(
                ...
                type="messages",
                avatar_images=(
                    None, (assets_path / "zgc3_logo.png").as_posix()),
                elem_id="chat-window"
            )
            ...
            with gr.Row(variant="panel"):
                clear_btn = gr.Button(
                    "🗑️ 新对话", variant="secondary", scale=0, min_width=120)
                ...
                submit_btn = gr.Button(
                    "🚀 发送", variant="primary", scale=0, min_width=120)
                ...
                with gr.Row():
                    with gr.Column(scale=1, min_width=300):
                        search_box = gr.Textbox(
                            label="关键词", placeholder="中关村三小", show_label=False)
                        search_btn = gr.Button("🔍 搜索 B 站", variant="primary")
                    with gr.Column(scale=3, min_width=500):
                        bili_panel = gr.HTML(
                            value="<div class='empty-state'>等待搜索...</div>")
        ...
        def handle_chat_submission(user_message: str, chatbot_ui_history: List[Dict[str, Optional[str]]], api_message_history: List[Dict[str, Any]]) -> Iterator[Dict[gr.component, Any]]:
            ...
            sources = []
            try:
                stream = orch.stream_ask_school(
                    user_message, api_message_history[:-1])
                for event in stream:
            ...
            if settings.enable_show_sources:
                final_answer += _format_sources_as_collapsible_markdown(
                    sources)
        ...
        submit_btn.click(handle_chat_submission, inputs=[
                         user_input, chatbot, api_history_state], outputs=[chatbot, api_history_state, user_input])
        user_input.submit(handle_chat_submission, inputs=[
                          user_input, chatbot, api_history_state], outputs=[chatbot, api_history_state, user_input])
        ...
        clear_btn.click(fn=clear_session, inputs=None, outputs=[
                        chatbot, api_history_state], queue=False)

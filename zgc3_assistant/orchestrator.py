...
class Orchestrator:
    ...
    def __init__(
        ...
        if self.rag_store is None:
            try:
                ...
                LOGGER.warning(
                    "RAG index not found at %s. RAG functionality will be disabled.", self.settings.rag_index_dir)
    ...
    def _collect_context(self, query: str, client: DashScopeClient) -> List[dict]:
        ...
        hits = self.rag_store.search(
            embeddings[0], top_k=self.settings.rag_top_k)
        if not hits:

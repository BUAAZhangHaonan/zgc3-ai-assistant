...
def test_rag_pipeline():
    ...
    if not orchestrator.rag_store:
        LOGGER.error("错误：RAG 知识库加载失败，请先运行 `python -m zgc3_assistant.rag.build_index`。")
        return
    ...
    coarse_pass_hits = orchestrator.rag_store.search(query_embedding[0], top_k=settings.rag_top_k)
    ...
    if not coarse_pass_hits:
        ...
        for i, hit in enumerate(coarse_pass_hits):
            print(f"  [{i+1}] Score: {hit['score']:.4f}, Source: {hit['source']}, Text: '{hit['text'][:100]}...'")
    ...
    context_text = "\n\n".join(f"[{idx+1}] {src['text']}" for idx, src in enumerate(reranked_hits))
